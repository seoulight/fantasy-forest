...
function scene:create( event )
	...
	local bird = display.newImageRect("image/char/bird_default.png", 60, 75)
	...
	-- 인어와 대화할 때 사용 (탭 이벤트가 겾쳐서 추가로 선언) --
	...
	nero2.alpha = 0
	transition.fadeIn( nero2, { delay = 2000, effect = "fade", time = 900 } )
	...
	local function removeItems()
		-- 틀릭하면 대화창 사라짐 --
		transition.fadeOut( nero2, { effect = "fade", time = 700 } )
		transition.fadeOut( text2, { effect = "fade", time = 700 } )
		transition.fadeOut( nName, { effect = "fade", time = 700 } )
		transition.fadeOut( txt, { effect = "fade", time = 700 } )
